        ...
        <tr v-for="coupon in coupons" :key="coupon.couponId" class="hover:bg-gray-50 border-t">
          ...
          <td class="px-4 py-3 text-right">
            <div class="flex justify-end gap-2">
              <NuxtLink :to="`/Coupon/code/${coupon.couponCode}?edit=true`" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-sm">Edit</NuxtLink >
              <NuxtLink :to="`/Coupon/code/${coupon.couponCode}?deleted=true`" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-sm">Delete</NuxtLink >
            </div>
          </td>
        </tr>
      </tbody>
...
const { data, pending, error: fetchError } = await useFetch<Response & { result: CouponResponse[] }>(config.public.apiBase + "/api/CouponAPI");
